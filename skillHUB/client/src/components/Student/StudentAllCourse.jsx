    ...
    return (
        ...
        <MDBCardGroup className='d-flex flex-wrap' style={{padding:"4px",display:'flex',gap:"10px",marginTop:"5px"}}>
            ...
            <MDBCardText  style={{display:'flex',justifyContent:'space-between'}}>
                <Link style={{padding:"5px",color:"red",fontSize:'large',boxShadow:'1px 1px 4px red'}} to={`cours/${c._id}`}>edit</Link>
                <Link   to={`cours/${c._id}`}><box-icon color='red' type='solid' name='cart'></box-icon></Link>
            </MDBCardText>
            </MDBCardBody>
            </MDBCard>
                ))}
